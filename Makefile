...
RAYLIB_INCLUDE_PATH   ?= src/include
...
ifeq ($(PLATFORM),PLATFORM_WEB)
    RAYLIB_LIB_PATH       ?= $(RAYLIB_PATH)/web
    PROJECT_BUILD_PATH    ?= build/web
    RAYLIB_RELEASE_PATH   ?= $(RAYLIB_PATH)/web
else
    RAYLIB_LIB_PATH       ?= $(RAYLIB_PATH)/desktop
    PROJECT_BUILD_PATH    ?= build/desktop
    RAYLIB_RELEASE_PATH   ?= $(RAYLIB_PATH)/desktop
endif
...
BUILD_WEB_SHELL       ?= src/shell.html
BUILD_WEB_HEAP_SIZE   ?= 67108864
...
ifeq ($(PLATFORM),PLATFORM_WEB)
    ifeq ($(OS),Windows_NT)
        ...
        ifeq ($(UNAMEOS),Linux)
            ...
            EMSDK_PATH         ?= /home/joao/Applications/emsdk
            EMSCRIPTEN_PATH    ?= $(EMSDK_PATH)/upstream/emscripten
            CLANG_PATH          = $(EMSDK_PATH)/upstream/bin
            PYTHON_PATH         = /usr/bin
            NODE_PATH           = $(EMSDK_PATH)/node/18.20.3_64bit/bin
...
ifeq ($(PLATFORM),PLATFORM_WEB)
    CFLAGS += -s EXPORTED_RUNTIME_METHODS=ccall -s STACK_SIZE=16777216
endif
